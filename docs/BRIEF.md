...
| Auto-zoom is two stages on two signals: **clicks decide when** (300 ms pre / 2500 ms post / 2500 ms merge, `zoom.ts`, STC-325); **change decides where** — amended from "movement decides where" once STC-326 built it: the region of the frame that changed near the event is the PRIMARY signal (`zoom-change.ts`'s `deriveZoomCrop`, reading the frame-difference sidecar STC-322 writes), and greedy dead-zone clustering of cursor movement (0.5 / 0.7 of the visible viewport) is now the FALLBACK for when the change track is missing or uninformative — true of every take today, since nothing has run the browser decode pass that writes `changes.json`. They are not entangled in one pass; a manual override (STC-330) wins over both | **built** | `transform/src/zoom.ts` (when), `transform/src/zoom-change.ts` (where — both signals), `render.ts` blends the result exactly as a manual override's target is blended. `docs/STC-326-RUNBOOK.md`: `gate:identity` with both stages has never run, and the classifier's thresholds are reasoned from this ticket's own stated outcomes rather than from STC-319's rules list, which still does not exist |
