...
func parseStart(_ cmd: [String: Any]) -> String {
    ...
    case .success(let r):
        ...
        return "ok:dir=\(r.dir):display=\(d):window=\(w):region=\(rg):camera=\(r.camera):mic=\(mic):cameraUid=\(cam)"
    case .failure(let e):
        return "err:\(e.code)"
    }
}
...
check("no scope fields at all keeps phase-1 behaviour: whole display, none requested",
      parseStart(["dir": "/tmp/x"]), "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:cameraUid=-")
check("a displayId alone is the phase-1 display scope",
      parseStart(["dir": "/tmp/x", "displayId": 7]),
      "ok:dir=/tmp/x:display=7:window=-:region=-:camera=false:mic=-:cameraUid=-")
check("camera is carried alongside any scope",
      parseStart(["dir": "/tmp/x", "camera": true]),
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=true:mic=-:cameraUid=-")
check("a region on its own display is a region scope",
      parseStart(["dir": "/tmp/x", "displayId": 3,
                  "region": ["x": 10, "y": 20, "width": 300, "height": 200]]),
      "ok:dir=/tmp/x:display=3:window=-:region=10.0,20.0,300.0,200.0:camera=false:mic=-:cameraUid=-")
check("a windowId alone is a window scope",
      parseStart(["dir": "/tmp/x", "windowId": 42]),
      "ok:dir=/tmp/x:display=-:window=42:region=-:camera=false:mic=-:cameraUid=-")
...
check("a malformed displayId falls back to the phase-1 default rather than being refused",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:cameraUid=-")
...
check("a micDeviceUid is carried through unexamined",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=BuiltInMicrophoneDevice:cameraUid=-")
...
check("a cameraDeviceUid is carried through unexamined",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:cameraUid=0x1234ABCD")
