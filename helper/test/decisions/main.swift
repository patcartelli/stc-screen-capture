...
// ── what a `start` request means (STC-370, STC-233) ─────────────────────────
func parseStart(_ cmd: [String: Any]) -> String {
    ...
    case .success(let r):
        ...
        return "ok:dir=\(r.dir):display=\(d):window=\(w):region=\(rg):camera=\(r.camera):mic=\(mic):sys=\(r.systemAudio)"
    case .failure(let e):
        return "err:\(e.code)"
    }
}
...
check("no scope fields at all keeps phase-1 behaviour: whole display, none requested",
      parseStart(["dir": "/tmp/x"]), "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:sys=false")
check("a displayId alone is the phase-1 display scope",
      parseStart(["dir": "/tmp/x", "displayId": 7]), "ok:dir=/tmp/x:display=7:window=-:region=-:camera=false:mic=-:sys=false")
check("camera is carried alongside any scope",
      parseStart(["dir": "/tmp/x", "camera": true]), "ok:dir=/tmp/x:display=-:window=-:region=-:camera=true:mic=-:sys=false")
check("a region on its own display is a region scope",
      parseStart(["dir": "/tmp/x", "displayId": 3,
                  "region": ["x": 10, "y": 20, "width": 300, "height": 200]]),
      "ok:dir=/tmp/x:display=3:window=-:region=10.0,20.0,300.0,200.0:camera=false:mic=-:sys=false")
check("a windowId alone is a window scope",
      parseStart(["dir": "/tmp/x", "windowId": 42]),
      "ok:dir=/tmp/x:display=-:window=42:region=-:camera=false:mic=-:sys=false")
...
check("a malformed displayId falls back to the phase-1 default rather than being refused",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:sys=false")
...
check("a micDeviceUid is carried through unexamined",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=BuiltInMicrophoneDevice:sys=false")
...
check("systemAudio true is carried through",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:sys=true")
...
check("a non-boolean systemAudio is off, not a crash — the same latitude camera gets",
      ...
      "ok:dir=/tmp/x:display=-:window=-:region=-:camera=false:mic=-:sys=false")
check("system audio is independent of the mic, and of a window scope",
      parseStart(["dir": "/tmp/x", "windowId": 42, "micDeviceUid": "BuiltInMicrophoneDevice",
                  "systemAudio": true]),
      "ok:dir=/tmp/x:display=-:window=42:region=-:camera=false:mic=BuiltInMicrophoneDevice:sys=true")
