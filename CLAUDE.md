...
| `transform/src/narration-clean.ts`, `scripts/clean-narration-one.mjs` | clean narration (STC-455), a SPIKE: nothing in the app calls it yet. One strength (0..1, 0 = exact identity) drives an 80 Hz high-pass and one shared STFT carrying noise reduction against a profile learned from the take's own pauses, late-reverb suppression (assumed 0.5 s room) and a de-esser. Pure TypeScript, whole track in, whole track out, run BEFORE the mix. The script writes `original.wav` + `clean-<pct>.wav` from a real `mic.m4a` (via `afconvert`) so it can be judged by ear first. `docs/STC-455-RUNBOOK.md` |
