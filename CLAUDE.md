...
| `transform/src/audio-mix.ts` | the export's audio mix (STC-418 PR 3): `mic + system × level`, hard-limited, one stereo 48 kHz track. Pure TypeScript, so it's Node-tested sample by sample. Exact passthrough at 48 kHz; linear resampling only for a 44.1 kHz mic. `export.ts` uses it ONLY when the take has `system.m4a`; mic-only exports are untouched. The level is a compact slider in the editor's timecode row, not a lane. The preview plays the same mix since STC-454 (`preview-audio.ts`) |
| `transform/src/preview-audio.ts`, `app/src/narration-worker.ts` | preview audio (STC-454): the editor preview plays the EXPORT's mix (`mixBlock`, 100 ms chunks scheduled 0.4 s ahead on Web Audio), at 1x only, silent while dragging (sound resumes `SEEK_SETTLE_MS` after a seek during playback), with a mute button that is an app setting (`settings.ts`'s `previewMuted`), never a take edit. While sound plays the PICTURE follows the AUDIO clock (`getOutputTimestamp`) through `preview.ts`'s `PlaybackAudio` hook — one clock cannot drift from itself. A late chunk is trimmed to start on time, never played late. Narration cleanup is applied in the worker so a long take does not stall the transport. Confirmed by ear 2026-09-25. Part 2: every audio control lives in ONE "Audio" popover in the editor's timecode row (Mic, System audio, Clean up voice), both levels shown in dB (`formatLevelDb`), and the mic may BOOST to +12 dB (project-11 `micLevel`, as recorded at 75% of the slider). Part 3: a speaker per track in that popover mutes it (project-12 `micMuted`/`systemAudioMuted`, its OWN field so the level is kept); a muted track is LEFT OUT of the export (`exportAudioPlan`'s `mic`/`system`), and every track muted exports with no audio track at all. `docs/STC-454-RUNBOOK.md` |
| `transform/src/waveform.ts` | the ruler's audio waveform (STC-454 part 4): `mixPeaks` runs `mixBlock` itself over the tracks the preview plays, at `previewLevels()` (editor.ts, the ONE reader of level+mute for sound and drawing), so what is drawn is the export's mix — cleaned mic, levels, mutes, hard limit. Linear peaks, 1024 buckets over the full take; a generator the editor drains in 8 ms slices. A ruler overlay behind its own toggle, never a lane (STC-444) |
| `transform/src/narration-clean.ts`, `scripts/clean-narration-one.mjs` | clean narration (STC-455). One strength (0..1, 0 = exact identity) drives an 80 Hz high-pass and one shared STFT carrying a decision-directed Wiener noise gain (profile learned from the take's own pauses — power subtraction was tried first and "bubbled") and a de-esser. No echo stage: tried twice, never audible, split out as STC-458. Pure TypeScript, whole mic track in and out, run BEFORE the mix. Wired in: project-10 `narrationCleanup: { enabled, strength }` (off at 0.5 by default), `audio-mix.ts`'s `exportAudioPlan` routes a cleaned mic through the mixer (mic-only takes included; off keeps the old path byte for byte), and the editor's "Clean up voice" switch + strength in the timecode row. The preview plays the cleaned mic too (STC-454), cleaned in `app/src/narration-worker.ts` off the main thread. The script writes `original.wav` + `clean-<pct>.wav` from a real `mic.m4a` (via `afconvert`) for A/B by ear. `docs/STC-455-RUNBOOK.md` |
...
| `transform/src/recording-profile.ts` | the bar's optional recording-profile selector (STC-447) — `RECORDING_PROFILES` (Instagram 1080px, 4K case study 3840px, both plain WIDTH targets through `outputSizeFor`, never a capture-side crop), `profileById` (unknown/null both mean "no profile"), and `outputSizeForProfile`, which CAPS the target to the capture's own width rather than the embed presets' `upscales: true` — there is no UI moment here to show a disabled option in. Also `PROFILE_HINT_FILE`'s name and doc comment: why `main.ts` never builds a real `Project` for this (importing `trim.ts` there fails `tsconfig.node.json`'s no-DOM pass through `transform-version.ts` → `cursor-art.ts`, the identical trap `project-version.ts` was split out to avoid) and instead drops two plain numbers `editor.ts` reads on a take's first open |
| `app/src/recording-profile-menu.ts` | the selector's menu template (STC-447) — pure, same position `thumbnail-menu.ts`/`tray-menu.ts` are in: nothing in Electron reads a `Menu` back once popped, so `buildRecordingProfileMenu` (checked-list "No profile" + the two built-ins) is the only version of this a test can see. Popped by `main.ts`'s `recorder:profileMenu`; the renderer, not the handler, turns a picked id into a `recordingProfileId` patch through the existing `recorder:setSettings` channel |
| `docs/STC-447-RUNBOOK.md` | what only a Mac can settle for the bar's visual pass and the recording-profile selector: whether the pill's new ring/shadow reads as legible-against-anything or as noise, the menu and sticky label round-tripping, and that a profiled take really opens sized for where it's headed. Written with no `swiftc`/`xcrun` at all in this session's sandbox — `npm test` could not run; typecheck and the pure unit suite (929 tests via a throwaway no-rebuild vitest config) did |
...
| `docs/STC-432-RUNBOOK.md` | main window chrome (STC-432, scoped down to just this) — `main.ts`'s explicit `trafficLightPosition` and `index.html`'s `#title-row` drag region + `--traffic-light-offset`, replacing a default lights position that collided with the row's own content. What only a Mac can settle: whether the 20/24 light position and 62px heading offset actually clear at default and minimum window width, whether the drag region (including double-click-to-zoom) feels native, and whether pill collapse/restore still returns the lights untouched. Written with no Xcode/swiftc, so only typechecked and bundled, never watched |
| `docs/STC-433-RUNBOOK.md` | display/mic fallback (STC-433) — `main.ts`'s `recorder:start` now checks the stored `displayId`/`micDeviceUid` against `sup.devices()` before the countdown, offering a `dialog.showMessageBox` choice instead of a raw refusal (display) or a silent no-audio take (mic). Display can fall back to Automatic; mic can only be turned off or re-chosen, never auto-picked (STC-233). What only a Mac can settle: whether the dialog actually appears and reads right, whether Automatic/Turn-Off actually retries and records, and whether a real CoreAudio stall correctly skips the mic check. Written with no Xcode/swiftc, so only typechecked and bundled, never watched |
...
| `docs/ELECTRON-REVIEW-KIT.md` | the codebase review prompt sequence for the Electron app (STC-465) — a shared preamble, steps 0–6 each writing `docs/review/<date>/step-N.md`, and a model-routing table. Written for THIS repo: its duplication/drift pass explicitly excludes the pure-decisions split and the tsconfig-driven splits, which a generic "one-callsite abstraction" pass flags as defects. Its calibration section names three findings (STC-466/467/468) the first run must re-find |
