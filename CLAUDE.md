...
| `app/src/scope-indicator.ts` | the persistent scope indicator's one pure decision (STC-381) — `resolveIndicatorTarget(scope, displays, liveWindow)`, no Electron and no window. Region scope converts its display-local rect to global points; window scope takes a FRESH lookup of the window's live bounds (never the bounds captured at pick time) and refuses to draw if that lookup came back empty — a closed window is "don't draw," never a stale rect. Screen scope always answers null: the ticket's own words are "there's nothing extra to confirm" |
| `app/src/scope-indicator-window.ts` | the indicator's real `BrowserWindow` mechanics, same split `pill.ts`/`pill-window.ts` uses. ONE window, not one per display (unlike `overlay-session.ts`) — there is no drag to cross a bezel, so the window itself IS the target rect (`setBounds` to it directly; `scope-indicator.html` draws a border at its own edge, no coordinate translation). `attachScopeIndicator` wires it to the main window's `focus`/`blur`; `scopeIndicatorScopeChanged` re-evaluates on any scope-affecting settings write (a kind change, a pick, a Clear) so a change while still focused clears or re-targets the outline at once rather than waiting for a blur/refocus round trip; `hideScopeIndicatorForRecording` is called as the very first line inside `recorder:start`, before the helper is ever touched — "closed before `start()`," the ticket's own words for why this needs none of the selection overlay's exclude-and-hide belt-and-braces (STC-290). `reconcile` also refuses to draw while `sup.state` is `"recording"`/`"starting"`, the one path a focus event can still fire mid-take (clicking the collapsed pill) |
| `app/renderer/scope-indicator.html` | the outline's whole view — no script, no preload, no IPC. Main sets the window's bounds directly to the target rect; the page's only job is a `border` at its own edge, coloured to match the app's existing `--accent` token (`#3b6fe0` light / `#7aa2ff` dark) rather than inventing a second accent |
| `app/test/scope-indicator.test.ts` | `resolveIndicatorTarget` exercised with no window: region-to-global conversion, refusing a display that no longer exists, refusing an unpicked target, refusing a window the fresh lookup didn't find (or found a DIFFERENT id for), and picking the display a straddling window's CENTRE falls on rather than its top-left corner |
| `app/test/scope-indicator.e2e.test.ts` | the indicator wired through the real app, against `_fake-helper.mjs`'s Finder stand-in (STC-381) — **this sandbox's Xvfb has no window manager** (the same limit `pill.e2e.test.ts` documents), so rather than click through the desktop to focus/blur the window, it calls `win.emit("focus")`/`emit("blur")` directly on the real `BrowserWindow`: not a simulation, the exact call Electron's native layer makes when a focus event fires for real, and `attachScopeIndicator`'s listeners cannot tell the difference. Proves the mechanism — appears on focus only with a resolved target (never for Screen scope), disappears on blur, disappears on a scope change or Clear even while still focused, disappears within one IPC round trip of Record — never the indicator's on-screen geometry, which needs a Mac |
| `docs/STC-381-RUNBOOK.md` | what only a Mac can settle for the scope indicator: whether the outline reads as helpful or naggy appearing on every focus (the ticket's own open question), whether the accent-blue border is visible against a busy desktop or a dark app behind it, and a live window-scope target actually tracking a real resize/move rather than the fixed rect a test fixture holds still |
...
| STC-381 | **persistent scope indicator — written 2026-09-15, on Linux, so the LOOK IS UNSEEN.** One of the three findings STC-374 filed rather than fixed: after picking a window or area, nothing on screen showed where it actually was — only a text readout in the profile sheet. The ticket's own "Decided approach" (written by its author before this session started) is what shipped as designed, not re-derived: a second, non-interactive `BrowserWindow` outlining the current scope's target, shown whenever the main window is FOCUSED and Scope is Window/Area with a resolved target, hidden on blur, on any scope/Clear change, and on Record — closed before `start()` so it can never end up in a recording. Split the same way `pill.ts`/`pill-window.ts` are: `scope-indicator.ts` is the one pure decision (`resolveIndicatorTarget`, testable with no window), `scope-indicator-window.ts` is the real mechanics. **One window, not one per display** — unlike the selection overlay (STC-290), there is no drag that can cross a bezel, so the indicator window IS the target rect (`setBounds` to it directly) and `scope-indicator.html` needs no coordinate translation, just a border at its own edge. A window-scope target is re-resolved from a FRESH `windows` lookup every time it is shown, per the ticket's "tracks the window's live position, not a stale snapshot from pick time" — a lookup that comes back empty (closed window) means don't draw, never a stale rect. `reconcile` also refuses to draw while `sup.state` is `"recording"`/`"starting"`, a case the ticket's text does not name but its own "must never end up in the capture" principle requires: focus can still land on the collapsed pill mid-take. The outline is a solid 3px border in the app's existing `--accent` token (light/dark), the one design choice the ticket left open — asked and answered before writing any code, rather than guessed. **Verification: 10 pure tests** (region-to-global conversion, refusing an unpicked or since-vanished target, refusing a DIFFERENT window than the one named, picking the display a straddling window's centre falls on) **plus 5 E2E tests driving the real app** against `_fake-helper.mjs`'s Finder stand-in — appears on focus only with a target, never for Screen scope, disappears on blur/scope-change/Clear/Record. **This sandbox's Xvfb has no window manager** (the same limit `pill.e2e.test.ts` already documents), so rather than clicking through the desktop to focus/blur the window, the E2E calls `win.emit("focus")`/`emit("blur")` directly — the exact call Electron's native layer makes when a focus event fires for real, which the attached listeners cannot tell apart from the real thing — and asserts the mechanism only, never on-screen geometry. Full local run (`xvfb-run`, rebuilt `app/dist/` first): every e2e file green except the same pre-existing, already-documented sandbox limits (`shell`/`frame-png` need `swiftc`; `manage.e2e.test.ts`'s macOS-only Trash-path assertion) — none touch this ticket's files. `npm run typecheck` clean on all three passes | a Mac: `docs/STC-381-RUNBOOK.md`. The outline has never been seen, and the ticket's own open question — whether appearing on every focus reads as helpful or naggy — is a Mac call this sandbox cannot make |
