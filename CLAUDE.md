...
| `docs/STC-433-RUNBOOK.md` | display/mic fallback (STC-433), rewritten 2026-09-25 after two events changed the underlying behavior: PR #221's merge ported it onto STC-388's fresh-overlay-pick Record flow (dropping "Choose a Different…" — there's no sticky picker left to focus), and a mid-recording mic-disconnect was found ending the WHOLE take (video included) with no dedicated reaction at all. `CaptureSession.handleMicDisconnected` (`helper/src/Capture.swift`, wired from `main.swift`'s `boot()`) now tears down ONLY the mic subsystem on a live disconnect, leaving video running — unlike a display change mid-take, which is still an intentional full stop (`AVAssetWriter` can't resize mid-file). `STC_CAPTURE_FAULT=mic-disconnected` reproduces it deterministically; no grant test exists yet (`tools/test-host` would need to forward the env var into the helper it spawns). Root cause of the original crash-like behavior is UNCONFIRMED — inferred from static reading, no crash log, no Mac in this session. What only a Mac can settle: everything — the pre-start dialogs (§1-§3) and, most importantly, whether §4's fix actually keeps video running when the mic dies mid-take, or whether the take still ends and a real crash log is needed |
