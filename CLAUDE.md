...
| `docs/STC-432-RUNBOOK.md` | main window chrome (STC-432, scoped down to just this) — `main.ts`'s explicit `trafficLightPosition` and `index.html`'s `#title-row` drag region + `--traffic-light-offset`, replacing a default lights position that collided with the row's own content. **First hardware pass (2026-09-25) found two real issues, both fixed**: the 10px heading gap read as none at all (widened to 16px, offset now 68px), and the drag region stopped at `#title-row`'s own top edge, leaving the 20px of window ABOVE it (`body`'s own top padding) undraggable — that padding moved onto `#title-row` itself so the drag region now starts at the window's actual top. Light position, double-click-to-zoom and pill collapse/restore were already confirmed correct. The fix itself is unwatched — still typechecked/bundled only |
| `docs/STC-433-RUNBOOK.md` | display/mic/camera fallback (STC-433), rewritten 2026-09-25 after events that changed the underlying behavior: PR #221's merge ported it onto STC-388's fresh-overlay-pick Record flow (dropping "Choose a Different…" — there's no sticky picker left to focus); a mid-recording mic-disconnect was found ending the WHOLE take (video included) with no dedicated reaction at all; and the camera was found to have the identical gap the same day, as a same-PR follow-up. `CaptureSession.handleMicDisconnected` / `handleCameraDisconnected` (`helper/src/Capture.swift`, both wired from `main.swift`'s `boot()`) each tear down ONLY their own subsystem on a live disconnect, leaving video running — unlike a display change mid-take, which is still an intentional full stop (`AVAssetWriter` can't resize mid-file). The two uid checks are deliberately NOT the same shape: the mic compares against the request (`wantMicUid`, always an exact uid — STC-233), but the camera's common "automatic" pick (STC-414) carries no uid in the request, so `CameraCapture` gained `currentDeviceUid()` (mirroring its existing `deviceName` tracking) and the camera compares against that — the device actually resolved to, not asked for. `STC_CAPTURE_FAULT=mic-disconnected` / `=camera-disconnected` reproduce each deterministically; no grant test exists yet (`tools/test-host` would need to forward the env var into the helper it spawns). Root cause of the original crash-like behavior is UNCONFIRMED for both — inferred from static reading, no crash log. `helper/build.sh` compiles and signs cleanly in a session with `swiftc` (6.4, via swiftly) and the binary passes a bare `status` smoke test, but neither fix has been watched against a real disconnect. What only a Mac can settle: everything — the pre-start dialogs (§1-§3) and, most importantly, whether §4/§5's fixes actually keep video running when the mic or camera dies mid-take, or whether the take still ends and a real crash log is needed |
