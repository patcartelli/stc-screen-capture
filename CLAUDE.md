...
| `docs/STC-381-RUNBOOK.md` | what only a Mac can settle for the confirmation flash, now on its THIRD design (persistent → 2000ms flash → 450ms hard-cut flash → hold-then-fade): whether `FLASH_HOLD_MS`/`FLASH_FADE_MS` (200/250) read right, whether the fade itself looks smooth or stutters, whether the accent-blue border is visible against a busy desktop, and that a freshly-picked window's real bounds are what actually gets outlined |
