...
| `app/src/scope-indicator-window.ts` | the indicator's real `BrowserWindow` mechanics, same split `pill.ts`/`pill-window.ts` uses. ONE window, not one per display (unlike `overlay-session.ts`) — there is no drag to cross a bezel, so the window itself IS the target rect. **Redesigned same-day from the ticket's original "shown on every main-window focus" — CONFIRMED ON HARDWARE to read as naggy rather than helpful.** `flashScopeIndicator` is the only way this is ever shown now: called once, from `pickCaptureTarget`, right after a fresh pick resolves, and it hides itself again after `FLASH_HOLD_MS` (450ms — down from an original 2000 that read as lingering on real hardware) with no further input — no focus/blur wiring to the main window at all. `hideScopeIndicator` cancels an in-progress flash early, from `recorder:setSettings` on any scope-affecting write (a kind change, a Clear) and as the first line inside `recorder:start`, before the helper is ever touched. Needs no `sup`/`BrowserWindow` reference of its own: `pickCaptureTarget` already refuses to run mid-recording, so nothing here can fire during a take by construction |
...
| `app/test/scope-indicator.e2e.test.ts` | the flash wired through the real app, against `_fake-helper.mjs`'s Finder stand-in (STC-381) — appears the instant a window or area is picked (never for Screen scope, which has no pick flow), clears itself after `FLASH_HOLD_MS` with no further input, and is cancelled EARLY by a scope change, a Clear, or Record, each asserted well inside the hold window so a test passing by outliving its own assertion isn't mistaken for the cancel actually firing. Never the indicator's on-screen geometry or how the hold duration feels, which need a Mac |
| `docs/STC-381-RUNBOOK.md` | what only a Mac can settle for the confirmation flash: whether `FLASH_HOLD_MS` (450, already moved down once from an original 2000) is long enough to register or still lingers, whether the accent-blue border is visible against a busy desktop, and that a freshly-picked window's real bounds are what actually gets outlined |
...
| STC-381 | **scope indicator — first cut written 2026-09-15 on Linux (LOOK UNSEEN), REDESIGNED TWICE THE SAME DAY from real-hardware feedback.** One of the three findings STC-374 filed rather than fixed: after picking a window or area, nothing on screen showed where it actually was. The first cut shipped the ticket's own "Decided approach" as written — a non-interactive outline shown on every main-window FOCUS, for as long as Scope stayed Window/Area. **Tested on real hardware and reported naggy, not helpful** — the ticket's own "Open" question, answered by living with it rather than guessed. Redesigned to a brief CONFIRMATION FLASH instead: `flashScopeIndicator` (in `scope-indicator-window.ts`) is now the only way the outline is ever shown, called once from `pickCaptureTarget` right after a fresh pick resolves, and then gone with no further input. All the main-window `focus`/`blur` wiring from the first cut is deleted outright — `pickCaptureTarget` already refuses to run mid-recording, so nothing in the new design can fire during a take by construction, no `reconcile`/`sup.state` guard needed. **Tried again at 2000ms hold and STILL reported as lingering** — the overlay's own highlight already confirms the target while picking, so the flash only needs to register that the pick landed, not hold long enough to read at leisure; `FLASH_HOLD_MS` is 450 now, a near-instant blink rather than removed outright. `hideScopeIndicator` cancels an in-progress flash early: wired into `recorder:setSettings` (a scope-kind change or Clear right after a pick) and as the first line inside `recorder:start`. `scope-indicator.ts`'s pure decision (`resolveIndicatorTarget`) is UNCHANGED by either redesign — only the caller changed, from a focus-driven `reconcile` doing its own `listWindows()` lookup to `pickCaptureTarget` handing in the window bounds it already has from the SAME list it used to build `windowLabel`, so the flash needs no second helper round trip. Outline style (solid accent-colour border) was the one open call the FIRST cut's ticket left, asked and answered before writing code; both SUBSEQUENT redesigns (flash vs. persistent, then the hold duration) were live product calls made watching the app on hardware, not requests. **Verification: 10 pure tests unchanged**, **6 E2E tests rewritten** for the flash contract, stable across repeated runs at the shortened duration — appears the instant a pick resolves (never for Screen scope, which has no pick flow), clears itself after `FLASH_HOLD_MS` with zero further input, and is cancelled EARLY (well inside the hold window, not just by outliving it) by a scope change, a Clear, or Record. Full local e2e sweep green (`xvfb-run`, rebuilt `app/dist/` first) on every file this ticket touches; `npm run typecheck` clean on all three passes | a Mac: `docs/STC-381-RUNBOOK.md`, rewritten for the flash design. Whether `FLASH_HOLD_MS` (450, already moved down once from 2000) now registers as a confirmation at all or is too quick to see, whether the accent-blue border is visible against a busy desktop, and that a freshly-picked window's real bounds (not a stale one) are what gets outlined |
