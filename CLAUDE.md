...
| STC-417 | **`frame-png.e2e.test.ts`'s "3 of 4 fail, known-bad" status was WRONG — DONE 2026-09-21, on real macOS hardware (swiftc present, real helper built and run).** Filed because the file's failures had become permanently ignorable, cited across a dozen other entries in this file as an expected baseline. The suspected shape in the ticket (STC-393's root-check widening) was a red herring — the actual cause is `app/test/_editor-fixture.ts`'s `launchApp` never isolating Electron's `userData` directory the way `camera-toggle.e2e.test.ts`'s own `--user-data-dir` pattern already does. Every other e2e isolation (`STC_RECORDINGS_DIR`, `STC_TEMP_TAKES_DIR`) was there; `userData` was not, so the real app read THIS DEVELOPER'S REAL `settings.json` — which has carried a real `still.destination: "/Users/pcartelli/Desktop"` since before STC-397's rename migration copied it forward. `still-io.ts`'s `destinationDir()` correctly (and per its own unit tests, deliberately) prefers a stored destination over a take's own directory, so every `#saveframe`/Copy-Shift-S in this file had been writing its PNG to the real Desktop instead of the fixture's take directory — found by grep-matching the exact filenames this run produced (`frame-2026-08-24_10-00-00-*ms.png`) sitting on `~/Desktop`, several going back to 2026-09-19. Not a product bug: `destinationDir` behaved exactly as designed and tested. Fixed with one line — `--user-data-dir=<fresh tmpdir>` in `_editor-fixture.ts`'s `launchApp`, mirroring `camera-toggle.e2e.test.ts` exactly. **4/4 now pass**, and all 13 e2e files that share this fixture (93 tests) re-run clean with no regressions. The stray PNGs this session's own reproduction run wrote to the real Desktop were deleted; older ones from prior sessions were left alone (not this session's to clean up, and not certainly attributable). This file's OTHER historical mentions of `frame-png.e2e.test.ts` as an expected failure (STC-373/374/326/329/380/393, the Linux-sandbox note near line 1347) are left as written — each was true on the Linux box it was measured from, for the unrelated and still-real reason that `shell.e2e.test.ts`/`frame-png.e2e.test.ts` need `STC_HELPER_BIN`/a real Swift binary neither sandbox had. This entry is what retires the "ignore these" framing going forward: on real hardware, with a built helper, the file is green | nothing — same latent `userData` leak likely affects the seven other e2e launchers still without `--user-data-dir` (`_quit-fixture.ts`, `export-identity.slow.test.ts`, `missing-helper`/`manage`/`product`/`shell`/`take-library`.e2e.test.ts`), unaudited here since none of them currently assert on where a still lands |
