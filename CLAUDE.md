...
| `docs/STC-382-WORKFLOW-STUDY.md` | the recording-vs-still workflow question (STC-382), as a POINT OF VIEW rather than a build — the ticket holds a design question with no acceptance criteria and this is the writeup it asked for. Reads both paths out of the code and names five divergences where the ticket named one; the load-bearing finding is that **the two actions have opposite completion models** (a still is finish-and-forget — the panel settles itself and saves; a recording is start-a-document — nothing opens, it waits in the library for trim/export/share), so the equal-weight button row asserts a symmetry the rest of the app does not have. Also: "Screen" resolves to `settings.displayId` for a recording and to the display under the POINTER for a still (coincides on a one-display Mac, untested on two); stills already carry their own scope vocabulary as three VERBS (`CAPTURE_ACTIONS`), so the Scope picker is a second differently-shaped answer rather than a missing one; three doors to a still against one to a recording. Recommends: label Scope as Record's now, differentiate the two buttons' weight next, DECLINE making Scope govern stills (it contradicts the completion model), and DEFER seeding a still from the sticky pick until STC-380 lands — seeding turns `windowLabel`'s deliberately cosmetic string into a live claim about a window that may be gone |
...
| STC-382 | **WRITEUP ONLY, no UI change — 2026-09-15, on Linux, and that is what the ticket asked for.** STC-382 holds a product-design question and says so in its own words ("this ticket exists to hold the question, not to prescribe an answer"); it has no acceptance criteria. Asked which deliverable was wanted rather than assuming, and the answer was the point of view first. `docs/STC-382-WORKFLOW-STUDY.md` is it. **The finding the ticket did not name, and the one the recommendation rests on: the two actions have OPPOSITE COMPLETION MODELS.** A still is finish-and-forget — the shot is on disk before anything is shown, the panel settles itself on the timeout and saves or copies, so doing nothing is a complete successful use (STC-296 by design). A recording is start-a-document — nothing opens when a take ends, the editor is reached only from the library's Open action, and trim/export/share are all still ahead of you. So the equal-weight two-button row asserts a symmetry the rest of the app does not have, and THAT is the source of the vagueness rather than the Scope wiring the ticket noticed. Four more divergences found reading the code: **"Screen" resolves to two different displays** (`settings.displayId` for a recording, `getDisplayNearestPoint(cursor)` for a still — they coincide on a one-display Mac, which is why nobody has met it, and NOBODY HAS RUN IT on two); stills already carry a scope vocabulary as three VERBS (`CAPTURE_ACTIONS` region/window/display, one hotkey and one menu item each), so the Scope picker is a SECOND differently-shaped answer rather than a missing one; three doors to a still (button, hotkey, menu bar) against one to a recording (no Record hotkey, no menu-bar Record); and `#lib-heading` statically says "Recordings" over a mixed grid. **Recommendation: label Scope as Record's now (cheap, truthful, reversible), differentiate the two buttons' weight next (needs an eye), DECLINE making Scope govern stills, DEFER seeding.** Governing is refused on merit — sticky framing is right for a session and wrong for a one-shot, and it would force finding B's two "Screen" rules into one control. Seeding is refused only on TIMING: it turns `windowLabel`'s deliberately cosmetic string into a live claim that the window still exists, on the very picker **STC-380 (open, PR #158)** is currently repairing — a good idea sitting behind another ticket's fix. Every cited `file:line` was re-checked against the tree rather than trusted from notes; three were wrong and corrected | a Mac, and the doc's §6 says which: whether a differentiated row reads as clearer or as inconsistent, whether the second row looks orphaned once Scope moves, and whether finding B is ever FELT — that needs two displays and a pointer on the non-primary one, and is a prediction, not a measurement. None of it gates the label change |
