...
| ticket | what | needs |
|---|---|---|
| STC-370 | **region and window scope for recordings — written 2026-09-14 on Linux, so NOTHING BELOW HAS RUN AGAINST REAL SCREENCAPTUREKIT.** `start` accepted only `displayId` (phase 1); it now also takes `region` (`{x,y,width,height}`, display-local points, `capture-still`'s own `crop` shape) or `windowId`, mutually exclusive, parsed once (`parseStartRequest`, the recording path's twin of `parseStillRequest`) before ScreenCaptureKit is ever touched — a malformed request costs no grant and no time, same as the still path. `resolveCaptureTarget` (Capture.swift) is the async half, mirroring `Still.swift`'s own `capture(content:)` switch, and reuses rather than re-derives: `chooseDisplay` (STC-247) for a display/region scope, `resolveCrop`/`framePixelSize` (STC-289) for the region's own rectangle, and a NEW `chooseDisplayForWindow` (CaptureDecisions.swift) for "which display is this window on" — factored out because `Still.swift` already had this exact logic inline, and a second copy is this file's most common defect. **`anchors.json` gains a `scope` block at a NEW version 3** — `CaptureScopeDoc` mirrors shot-1's kind/crop/window shapes on purpose. A whole-display take still writes version 2 with no `scope` block at all (`anchorsDocument` emits the MINIMUM version that can express the document, `scope: CaptureScopeDoc = .display` defaulting every pre-existing call site), so nothing about a normal take changed. **The mid-take decision the ticket asked for:** a window-scope take polls its own window once a second (`CGWindowListCopyWindowInfo` — SCK has no delegate for this the way it does for the stream dying); a size change past half a point ends the take cleanly (`stop.reason: "window-resized"`, mirroring how a display reconfiguration already ends one, because `AVAssetWriter` cannot change output dimensions mid-file either way); the window no longer being found ends it with `"window-closed"`; a plain MOVE is deliberately ignored, on the design assumption that `SCContentFilter(desktopIndependentWindow:)` follows a moving window and keeps delivering the same pixel size — the one claim in this ticket a Mac can prove wrong outright. `STC_CAPTURE_FAULT=window-resized`/`=window-closed` fire the watcher's reaction deterministically (same idiom as `=stream-died`), so the stop/sidecar-writing PATH is proven without needing to script a real window resize. `docs/STC-370-RUNBOOK.md` has the full account, including what the fault CANNOT prove (that a real resize is detected in the first place) | a Mac, entirely: `docs/STC-370-RUNBOOK.md` — region capture, window capture, a REAL resize/close, and whether a pure move really is safe. Blocks STC-374's profile-sheet scope picker (its own "Depends on" section names this ticket a prerequisite, not a detail) |
