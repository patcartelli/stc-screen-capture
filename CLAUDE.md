...
| STC-376 | **written 2026-09-14 on Linux, so NOT YET RUN against real ScreenCaptureKit — found and diagnosed live while verifying STC-370 on hardware, then fixed here.** `App.shutdown()`'s guard used to be `state == .recording \|\| state == .starting`, so a `stop` command already in flight (`state == .stopping`, teardown running async — `writeSidecars()`/`writer.finishWriting()` not yet done) fell through neither branch when a `quit`/stdin-close shutdown landed right behind it: the old code sent `bye` and called `exit()` immediately, before that teardown had written anything. Reproduced LIVE on hardware, not just reasoned about: a `display.mp4` existed on disk with no `moov` atom (QuickTime refused to open it) and no `anchors.json`/`events.json` at all, from the exact shell idiom several runbooks use (`echo stop` immediately followed by the pipe closing). Not specific to region/window scope — any take racing `stop` against the process exiting hits it. The fix is a wait, not a wider guard: `App` gained `idleWaiters` (an array of closures fired once by whichever `stop()` teardown is CURRENTLY in flight, the instant `state` next becomes `.idle`, guarded by its own lock since `shutdown()` can run on the signal-handling queue while `notifyIdle()` always runs on main from inside `stop()`'s completion). `shutdown()`'s guard is `state != .idle` now, and when it finds `state == .stopping` it registers itself as a waiter (`addIdleWaiter`) instead of calling `stop()` a second time — calling `stop()` again would hit ITS OWN "not recording" guard (`.stopping` is neither `.recording` nor `.starting`) and answer immediately, which is the same bug one layer in. `helper/test/capture.test.ts` gained a regression test that reproduces the race deterministically with no grant needed: `start` synchronously sets `state = .starting` and assigns `capture` before ScreenCaptureKit is ever touched, so the race lives entirely in that gap — sending `start`, `stop`, `quit` back to back (no sleeps) relies on `IO.readCommands` dispatching each line onto the main queue via its own `DispatchQueue.main.async`, in the order the lines were read, so `handle(stop)`'s synchronous `state = .stopping` is guaranteed to have already run by the time `handle(quit)` calls `shutdown()` — and asserts `events.json`/`anchors.json` exist and the `stop` reply still arrived before `bye` | a Mac: this has never been compiled or run against a real compiler. Expect the same 1-2 rounds of real `swiftc` feedback STC-370 needed before CI goes green |
