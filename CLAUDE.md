...
| STC-378 | **editor window: the export-grid ticks (STC-338 rule 9) could stay `hidden` forever after the FIRST take opened in a fresh editor window — a real regression from STC-373, found watching CI on an unrelated PR (#139/STC-376), not this ticket's own hardware.** `editor.ts`'s `updateTicks()` measures `$("timeline").getBoundingClientRect().width` synchronously as part of `openTakeOrThrow`'s boot sequence; `editor-window.ts`'s `openEditor()` creates the `BrowserWindow` with no `show: false`, so nothing guarantees the content view has its final layout size by the time that read happens. Reproduced identically on TWO separate CI runs at the same commit (`8043c4f`) — bisected via `list_workflow_runs`: green at the STC-372 merge, red starting at the STC-373 editor-window merge, still red after. **Could not be reproduced under Xvfb on this Linux sandbox despite a full local run plus dedicated stress loops (9 runs, 0 failures) — this sandbox's window manager apparently lays a window out immediately on creation, unlike whatever CI's macOS runner does under load.** So the fix is not a tuned delay (nothing here could tune one against real evidence): `updateTicks()` retries a bounded number of times via `requestAnimationFrame` before believing a non-positive width, since the existing `resize` listener only self-heals if a genuine LATER resize follows, which an explicitly-sized `BrowserWindow` need not ever produce. Same family as STC-338's "a measurement that cannot fail loudly will fail quietly and plausibly" and STC-318's "a measurement that cannot see what it was being read as evidence about" — this one had no single known reveal to measure after, so it measures again instead. The regression test injects the exact failure condition directly (`#timeline.getBoundingClientRect` monkey-patched to answer 0 width exactly once, then a real `resize` dispatched) rather than relying on the race actually occurring, and was watched FAILING against the pre-fix code before the fix was restored — a genuine discriminator, not a decoration. All 56 tests across every editor-touching E2E file (scrubber/export-size/export-write-guard/export/legibility/preview/share) pass locally with the fix | a Mac: this has never run against CI's real macOS window-layout timing, only against the bug report it produced twice. If it recurs, the retry count (5) or the animation-frame-based wait may need to grow, or the real culprit may be something Xvfb structurally cannot reveal |
