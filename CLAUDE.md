...
| `docs/STC-381-RUNBOOK.md` | what only a Mac can settle for the confirmation flash: whether `FLASH_HOLD_MS` (2000) is long enough to register or lingers, whether the accent-blue border is visible against a busy desktop, and that a freshly-picked window's real bounds are what actually gets outlined |
...
| STC-381 | **scope indicator — first cut written 2026-09-15 on Linux (LOOK UNSEEN), REDESIGNED THE SAME DAY from real-hardware feedback.** One of the three findings STC-374 filed rather than fixed: after picking a window or area, nothing on screen showed where it actually was. The first cut shipped the ticket's own "Decided approach" as written — a non-interactive outline shown on every main-window FOCUS, for as long as Scope stayed Window/Area. **Tested on real hardware the same day and reported naggy, not helpful** — the ticket's own "Open" question, answered by living with it rather than guessed. Redesigned to a brief CONFIRMATION FLASH instead: `flashScopeIndicator` (in `scope-indicator-window.ts`) is now the only way the outline is ever shown, called once from `pickCaptureTarget` right after a fresh pick resolves, up for `FLASH_HOLD_MS` (2000, untuned) and then gone with no further input. All the main-window `focus`/`blur` wiring from the first cut is deleted outright — `pickCaptureTarget` already refuses to run mid-recording, so nothing in the new design can fire during a take by construction, no `reconcile`/`sup.state` guard needed. `hideScopeIndicator` cancels an in-progress flash early: wired into `recorder:setSettings` (a scope-kind change or Clear right after a pick) and as the first line inside `recorder:start`. `scope-indicator.ts`'s pure decision (`resolveIndicatorTarget`) is UNCHANGED by the redesign — only the caller changed, from a focus-driven `reconcile` doing its own `listWindows()` lookup to `pickCaptureTarget` handing in the window bounds it already has from the SAME list it used to build `windowLabel`, so the flash needs no second helper round trip. Outline style (solid accent-colour border) was the one open call the FIRST cut's ticket left, asked and answered before writing code; the SECOND redesign (flash vs. persistent) was a live product call made watching the first cut on hardware, not a request. **Verification: 10 pure tests unchanged**, **6 E2E tests rewritten** for the flash contract — appears the instant a pick resolves (never for Screen scope, which has no pick flow), clears itself after `FLASH_HOLD_MS` with zero further input, and is cancelled EARLY (well inside the hold window, not just by outliving it) by a scope change, a Clear, or Record. Full local e2e sweep green (`xvfb-run`, rebuilt `app/dist/` first) on every file this ticket touches; `npm run typecheck` clean on all three passes | a Mac: `docs/STC-381-RUNBOOK.md`, rewritten for the flash design. Whether `FLASH_HOLD_MS` (2000) is long enough to register or lingers, whether the accent-blue border is visible against a busy desktop, and that a freshly-picked window's real bounds (not a stale one) are what gets outlined |
